...
controller.hears(['listall'], ['direct_message'], function(bot, message) {

    bot.api.users.list({ token: process.env.token }, function(err, res) {
        var users_list = [];
        if (err)
            console.log(err);
        else {
            ...
            for (var i = 0; i < res.members.length; i++) {
                if (!(res.members[i].is_bot) && (res.members[i].id != 'U1TN782GJ')) {
                    users_list.push(res.members[i].id);
            ...
            //bot.reply(message, users_list);
            console.log(users_list);
            //console.log(res.members[0]);
        }
    });
});
